...
export async function handler(req: Request, ctx: HandlerContext) {
  ...
  const result = await db.queryArray(`SELECT id, title, SUBSTRING(body, 1, $1) as body, user_id FROM post WHERE user_id=$2`, [postLength, ctx.params.userid]).finally(() => db.release());

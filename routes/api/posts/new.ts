...
export const handler: Handlers = {
  async POST(req: Request, _ctx: HandlerContext) {
    try {
      ...
      if(!cookies.has('token')) return new Response(createFromMessage('no authenication token'), {status: 401});
      const token = cookies.get('token');
      const body: ExpectedBody = await req.json();
      if(!body.title || !body.body) return new Response(createFromMessage('invalid post data'), {status: 400});
      const result = await writePost(body.title, body.body, +token!);
      if(result instanceof Error) return new Response(createFromError(result), {status: 400});

...
export const handler: Handlers = {
  async GET(req: Request, ctx: HandlerContext) {
    ...
    const fields = url.searchParams.has('fields') ? url.searchParams.get('fields')!.split(',') : undefined;
    if(fields) for(const field of fields) if(!(Post.Fields as string[]).includes(field)) return new Response(createFromMessage(`field ${field} is not valid`), {status: 400})
    const post = await getPostByID(+ctx.params.postid, fields as Post.Fields).catch(e => e);
